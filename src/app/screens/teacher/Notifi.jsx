...
import TeacherLeadsCard from "../../components/TeacherLeadsCard";
    ...
    <>
      <section className="w-full flex flex-col items-center">
        <div className="flex p-4 w-full font-bold text-2xl text-gray-800 items-center gap-2">
          <span>Student Follows : {user?.user_details?.followings.length}</span>
          <svg
            ...
            <table className="w-full text-sm text-left rtl:text-right text-gray-500 .text-gray-400">
              <caption className="p-5 text-lg font-semibold text-left rtl:text-right text-gray-900 bg-white .text-white .bg-gray-800">
                ...
                <p className="mt-1 text-sm font-normal text-gray-500 .text-gray-400">
                  students who apply as interested will be shown here
                </p>
              </caption>
              ...
              <tbody>
                {user?.user_details?.followings &&
                  user?.user_details?.followings.map((e,i) => (
                  <TeacherLeadsCard key={e.id} e={e}/>
                  ))}
              </tbody>
            </table>
          </div>
